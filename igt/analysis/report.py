"""Plain-text reporting for completed result-analysis runs.

The report writer combines validated model summaries, derived descriptive tables,
and population-level inference into a human-readable audit of the generated
analysis artifacts.
"""
...
from .plots import DISPLAY_CONFIGS


def _model_display_name(model_name: str) -> str:
    """Map a canonical model identifier to its report display label.

    Args:
        model_name: Canonical model identifier.

    Returns:
        A compact human-readable label derived from the canonical identifier.
    """

    model_key = model_name.strip().replace("  ", " ").replace(" ", "_").lower()
    display_config = DISPLAY_CONFIGS.get(model_key)

    if display_config is None:
        return model_name

    return display_config.get("name", model_name)
...
def _append_generated_artifacts(
    ...
    *,
    title: str,
    artifacts: tuple[GeneratedFigure, ...] | tuple[GeneratedTable, ...],
) -> None:
    """Append grouped logical artifact paths to the report."""

    lines.append(f"{title}: {len(artifacts)}")

    for artifact in artifacts:
        lines.append(f"  {normalize_path(artifact.output_stem).as_posix()}")

        for path in artifact.paths:
            format_name = path.suffix.lstrip(".").upper() or "FILE"
            lines.append(f"    {format_name}: {normalize_path(path).as_posix()}")
...
def write_analysis_report(
    ...
    generated_tables: tuple[GeneratedTable, ...],
    *,
    ...
    Args:
        summary: Validated aggregate model-summary table.
        model_win_table: AIC/BIC model-win count and rate table.
        study_preference: Per-study model-preference summary table.
        boundary_summary: Model-level parameter-boundary summary table.
        parameter_summary: Model-parameter summary table.
        criterion_inference: Population-level AIC/BIC difference inference table.
        model_win_inference: Population-level model-win inference table.
        generated_figures: Logical figure outputs generated by the analysis.
        generated_tables: Logical table outputs generated by the analysis.
        report_path: Destination path for the generated text report.
    """
    ...
    _append_generated_artifacts(
        lines,
        title="Figures",
        artifacts=generated_figures,
    )
    _append_generated_artifacts(
        lines,
        title="Tables",
        artifacts=generated_tables,
    )
